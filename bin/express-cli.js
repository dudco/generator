...
program
  ...
  .option("-l, --lang <language>", "langage (js, ts)")
  .parse(process.argv);
...
function createApplication(name, dir) {
  ...
  var pkg = {
    ...
    private: true,
    scripts: {
      start: program.lang === "js" ? "node ./bin/www" : "ts-node ./bin/www"
    },
    dependencies: {
      debug: "~2.6.9",
      express: "~4.16.1"
    },
    devDependencies: program.lang === "ts" && {
  ...
  // JavaScript
  var app = loadTemplate(`${program.lang}/app.${program.lang}`);
  var www = loadTemplate(`${program.lang}/www`);
  ...
  app.locals.mounts = [];
  app.locals.uses = [];
  if(program.lang === "ts") pkg.devDependencies["@types/node"] = "^11.9.4";

  // Request logger
  app.locals.modules.logger = "morgan";
  app.locals.uses.push("logger('dev')");
  pkg.dependencies.morgan = "~1.9.0";
  if(program.lang === "ts") pkg.devDependencies["@types/morgan"] = "^1.7.35";
  ...
  if(program.lang === "ts") pkg.devDependencies["@types/cookie-parser"] = "^1.4.1";
  if(program.lang === "ts") pkg.scripts["build"] = "tsc";
  ...
  // Static files
  app.locals.uses.push("express.static(path.join(path.__dirname, 'public'))");

  if (program.git) {
    copyTemplate(`${program.lang}/gitignore`, path.join(dir, ".gitignore"));
  }
  ...
  // write files
  write(path.join(dir, `app.${program.lang}`), app.render());
